...
```bash
uv run src/ingestion/samples/cloud_stroage.py

uv run src/ingestion/samples/local_file_system.py

uv run src/ingestion/samples/mysql.py
```
...
```bash
export DBT_PROJECT_DIR=src/transformation/sample/mysql
...
```bash
streamlit run src/visualization/samples/pygwalker/main.py
```
...
```bash
jupyter notebook src/experimentation/samples
```
